...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .route-btn:hover { background: #16392c; }
    .route-side { margin-top: 12px; }
    .route-result {
      font-size: 13px;
      color: var(--ink);
      line-height: 1.6;
    ...
    .route-result b { color: var(--accent); }
...
<body>
  <aside>
    ...
    <div>
      <h2 style="font-size:18px;margin:0 0 4px">Find route</h2>
      <p style="margin:0 0 8px;color:var(--muted);font-size:12px">A* shortest path between two cities (km).</p>
      ...
      <button id="findRouteBtn" class="route-btn" type="button">Find Route</button>
  ...
  <script>
    ...
    for (const e of G.edges) {
      ...
      line.dataset.isMst = e.kind === "mst" ? "1" : "0";
    ...
    function highlight(i) {
      ...
      if (i >= 0) {
        G.edges.forEach(e => {
          if (e.source === i) hoverNeighbors.add(e.target);
          else if (e.target === i) hoverNeighbors.add(e.source);
        });
      }
      redraw();
    ...
    // -- route state shared with redraw() --------------------------------
    let routeActive = false;
    let routeStart = -1, routeGoal = -1;
    const routePath = new Set();
    const routeEdges = new Set();

    function redraw() {
      edgeEls.forEach(el => {
        const a = +el.dataset.s, b = +el.dataset.t;
        const key = a < b ? a + "|" + b : b + "|" + a;
        const baseW = el.dataset.isMst === "1" ? "0.7" : "0.95";
        const hit = active >= 0 && (el.dataset.s == active || el.dataset.t == active);
        if (routeEdges.has(key)) {
          ...
          el.setAttribute("stroke-opacity", hit ? "0.5" : "0.10");
          el.setAttribute("stroke-width", hit ? "1.8" : baseW);
        } else {
          el.setAttribute("stroke", hit ? "#1f4d3a" : "#8a7d6b");
          el.setAttribute("stroke-opacity", active < 0 ? "0.45" : (hit ? "0.95" : "0.08"));
          el.setAttribute("stroke-width", hit ? "1.8" : baseW);
        }
      });
      nodeEls.forEach((el, idx) => {
        const hover = active >= 0 && (idx === active || hoverNeighbors.has(idx));
        let fill = "#9a3412", opacity = "0.95", stroke = "#f4efe6", sw = "0.6";
        if (idx === routeStart) { fill = "#14532d"; opacity = "0.98"; stroke = "#14532d"; sw = "2"; }
        else if (idx === routeGoal) { fill = "#c2410c"; opacity = "0.98"; stroke = "#c2410c"; sw = "2"; }
        else if (routePath.has(idx)) { fill = "#1f4d3a"; opacity = "0.95"; }
        else if (routeActive) { opacity = hover ? "0.55" : "0.12"; }
        else { opacity = (active < 0 || hover) ? "0.95" : "0.12"; }
    ...
    // -- A* route finding (port of project/search/astar.py) --------------
    const EARTH_KM = 6371.0;
    function haversineKm(lat1, lon1, lat2, lon2) {
      const r = Math.PI / 180;
      const p1 = lat1 * r, p2 = lat2 * r;
      const dphi = (lat2 - lat1) * r;
      const dlmb = (lon2 - lon1) * r;
      const a = Math.sin(dphi / 2) ** 2 + Math.cos(p1) * Math.cos(p2) * Math.sin(dlmb / 2) ** 2;
      return 2 * EARTH_KM * Math.asin(Math.sqrt(Math.min(1.0, a)));
    }
    ...
    function buildAdjacency() {
      ...
      for (const arr of adj.values()) arr.sort((x, y) => x - y);
      return adj;
    }
    const adjacency = buildAdjacency();
    const edgeKm = new Map();
    for (const e of G.edges) {
      edgeKm.set(e.source + "|" + e.target, e.km);
      edgeKm.set(e.target + "|" + e.source, e.km);
    }

    function compareHeap(a, b) { return a[0] - b[0] || a[1] - b[1]; }
    function heapPush(h, item) {
      h.push(item);
      let i = h.length - 1;
      while (i > 0) {
        const p = (i - 1) >> 1;
        if (compareHeap(h[p], h[i]) <= 0) break;
        [h[p], h[i]] = [h[i], h[p]];
        i = p;
      }
    }
    function heapPop(h) {
      ...
      if (h.length) {
        ...
        for (;;) {
          ...
          let m = i;
          if (l < h.length && compareHeap(h[l], h[m]) < 0) m = l;
          if (r < h.length && compareHeap(h[r], h[m]) < 0) m = r;
          if (m === i) break;
          [h[m], h[i]] = [h[i], h[m]];
          i = m;
        }
    ...
    // Same semantics as astar.py: heap by (f, counter), explored set,
    // best_g gate (re-queue on better g), goal checked on POP, neighbors
    // expanded in ascending id order (buildAdjacency sorts them).
    function aStagSearchJS(fromId, toId) {
      const startH = haversineKm(G.nodes[fromId].lat, G.nodes[fromId].lon,
        G.nodes[toId].lat, G.nodes[toId].lon);
      const startChain = { state: fromId, g: 0.0, parent: null };
      const frontier = [[startH, 0, startChain]];
      const best_g = new Map([[fromId, 0.0]]);
      const explored = new Set();
      let expanded = 0, generated = 1, counter = 0, maxFrontier = 1;
      while (frontier.length) {
        const [, , chain] = heapPop(frontier);
        const s = chain.state, g = chain.g;
        if (explored.has(s)) continue;
        if (s === toId) {
          const path = [];
          for (let c = chain; c; c = c.parent) path.push(c.state);
          path.reverse();
          return { status: "success", path, cost: g, expanded, generated, maxFrontier };
        }
        explored.add(s);
        expanded++;
        for (const nb of adjacency.get(s)) {
          generated++;
          ...
          if (!best_g.has(nb) || gc < best_g.get(nb)) {
            best_g.set(nb, gc);
            counter++;
            const f = gc + haversineKm(G.nodes[nb].lat, G.nodes[nb].lon,
              G.nodes[toId].lat, G.nodes[toId].lon);
            heapPush(frontier, [f, counter, { state: nb, g: gc, parent: chain }]);
            maxFrontier = Math.max(maxFrontier, frontier.length);
          }
        }
      }
      return { status: "failure", path: [], cost: 0, expanded, generated, maxFrontier };
    }
    ...
    function renderRouteSelects() {
      const cities = [...G.nodes].sort((a, b) => a.name.localeCompare(b.name));
      const from = document.getElementById("routeFrom");
      const to = document.getElementById("routeTo");
      for (const n of cities) {
        const o = document.createElement("option");
        o.value = n.id;
        o.textContent = n.name + " — " + n.state;
        from.appendChild(o);
        to.appendChild(o.cloneNode(true));
      }
      const tijuana = nodeById(6);
      const cancun = nodeById(25);
      if (tijuana) from.value = String(tijuana.id);
      if (cancun) to.value = String(cancun.id);
    }

    function paintRoute(path, startId, goalId) {
      routePath.clear();
      routeEdges.clear();
      for (const id of path) routePath.add(id);
    ...
    function runRoute() {
      ...
      const res = aStagSearchJS(fromId, toId);
      if (res.status === "success") {
        const a = nodeById(fromId), b = nodeById(toId);
        routeResult.innerHTML =
          "<b>" + a.name + " → " + b.name + "</b><br>" +
          "Cost: " + res.cost.toFixed(2) + " km<br>" +
          "Hops: " + (res.path.length - 1) + "<br>" +
          "Nodes expanded: " + res.expanded + "<br>" +
          "Nodes generated: " + res.generated;
        paintRoute(res.path, fromId, toId);
      } else {
        routeResult.textContent = "No route found";
        clearRoute();
      }
    }
    document.getElementById("findRouteBtn").addEventListener("click", runRoute);
    document.getElementById("routeFrom").addEventListener("change", () => {
      clearRoute();
      routeResult.textContent = "Adjust From/To, then press Find Route.";
    });
    document.getElementById("routeTo").addEventListener("change", () => {
      clearRoute();
      routeResult.textContent = "Adjust From/To, then press Find Route.";
    });
  </script>
